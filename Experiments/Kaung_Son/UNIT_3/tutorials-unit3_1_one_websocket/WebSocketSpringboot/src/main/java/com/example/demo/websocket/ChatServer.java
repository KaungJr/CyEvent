...
public class ChatServer {
    ...
    public void onOpen(Session session, @PathParam("username") String username) throws IOException {
        ...
        if (usernameSessionMap.containsKey(username)) {
            ...
            sendMessageToPArticularUser(username, "Welcome to our exclusive chat server, " + username + "! Let's start the conversation!");


            // send to everyone in the chat & Total users online after joining
            broadcast("User: " + username + " has Joined the Chat Total users online: " + sessionUsernameMap.size());
            // Send list of current users to the new user
            StringBuilder userList = new StringBuilder("Currently online: ");
            sessionUsernameMap.values().forEach(user -> userList.append(user).append(", "));
    ...
    public void onClose(Session session) throws IOException {
        ...
        // send the message to chat & total user online after dissconneting
        broadcast(username + " disconnected. Total users online:" + sessionUsernameMap.size());
    }
    ...
    private void broadcast(String message) {
        // Added timestamp in regular 12-hour format with AM/PM
